...
合わせて拡大され、左右がはみ出して切られるため、**必要な解像度は「枠の幅」ではなく「枠の高さ × DPR」で決まる**。

倍率 = max(枠幅 ÷ 画像幅, 枠高 ÷ 画像高) × DPR（1.00を超えると引き伸ばし）

実測した枠の大きさ（CSS px）：375px幅 335×251.3 / 393px幅 353×264.8 / 430px幅 390×292.5 /
820px幅 233×262 / 1440px幅 273×238。769px未満は1列表示で、枠は「画面幅−40px」×その3/4になる。

- `120.webp`：main と同じ 1400×788 のまま。元画像が1400pxしかなく、iPhone（DPR3）ではすでに
  ほぼ等倍のため縮小の余地がない。
  （当初このPRで1000×563に縮小したが、幅で計算した誤りで、iPhone DPR3で1.34〜1.56倍に引き伸ばされ、
  障子の格子・照明の編み目・畳の目が甘くなることがレビューで判明したため戻した。）
- `taiken.webp`：高さで上限を決め、1567×1045 → 1320×880。iPhone（幅430px以下）では枠の高さが最大292.5 CSS px、
  DPR3で878 device pxなので、880pxあれば等倍以下になる。
- `shizen.webp`：元が700×1050で、縮小の余地がない（変更なし）。

同じURLのまま中身だけ差し替えたので、CSSの記述は変更していない。`image-set()` は使っていない。

### DPR別の拡大倍率

| 画像 | 375@3x | 393@3x | 430@3x | 744@2x | 768@2x | 820@2x | 1440@2x |
|---|---:|---:|---:|---:|---:|---:|---:|
| 120.webp 1400×788（main・本PR共通） | 0.96 | 1.01 | 1.11 | 1.34 | 1.39 | 0.66 | 0.60 |
| taiken.webp 1567×1045（main） | 0.72 | 0.76 | 0.84 | 1.01 | 1.04 | 0.50 | 0.46 |
| taiken.webp 1320×880（本PR） | 0.86 | 0.90 | 1.00 | **1.20** | **1.24** | 0.60 | 0.54 |
| shizen.webp 700×1050（main・本PR共通） | 1.44 | 1.51 | 1.67 | 2.01 | 2.08 | 0.67 | 0.78 |

- iPhoneの幅（375〜430px）では、taiken.webp は main と同じく等倍以下。
- **1列表示のタブレット縦向き（744〜768px、iPad mini等・DPR2）では、taiken.webp が main の約1.0倍から1.2倍台に上がる。**
  iPhoneを優先した判断で、この範囲は画質が main より下がる。
- 120.webp（タブレット縦向き）と shizen.webp（スマホ全般）は元画像の解像度が足りず、main の時点で1倍を超えている。
  本PRによる変化はない。
...
| assets/images/taiken.webp | 126,838 | 96,594 | 1567×1045 → 1320×880 |
| assets/images/shizen.webp | 269,690 | 269,690 | 700×1050（変更なし） |
| assets/images/moji-1362.webp | （新規） | 29,842 | 1362×454 |

- 魅力カード3枚合計：582,608 → 552,364 bytes（30,244 bytes / 5.2%削減。全端末共通）。
- heroロゴ：DPR2の820px幅・1440px幅、DPR3の820px幅では 162,768 → 29,842 bytes（132,926 bytes削減）。
  375〜430px幅（iPhone）では従来どおり780pxのWebPが選ばれ、変化しない。
